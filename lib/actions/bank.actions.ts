...
import { plaidClient } from "../plaid"; //previously was : import { plaidClient } from "../plaid.config";
...
export const getAccount = async ({ appwriteItemId }: getAccountProps) => {
  try {
    if (!appwriteItemId || typeof appwriteItemId !== "string") {
      throw new Error("Invalid appwriteItemId passed to getAccount");
    }
    // get bank from db
    const bank = await getBank({ documentId: appwriteItemId });

    console.log("Received appwriteItemId:", appwriteItemId);
    ...
    throw error; // rethrow to avoid silent failure in calling components
  }
};
...
export const getInstitution = async ({
  ...
  try {
    const institutionResponse = await plaidClient.institutionsGetById({
      ...
      country_codes: ["US"] as CountryCode[], // "US" for sandbox
    });
...
{/*export const getTransactions = async ({ accessToken,}: getTransactionsProps) => {
  ...
  try {
    ...
    while (hasMore) {
      ...
      hasMore = data.has_more; // COMMENTED, ORIGINAL CODE
    }
